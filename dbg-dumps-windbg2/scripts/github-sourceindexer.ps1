...
.DESCRIPTION
  ...
  Src: https://github.com/Haemoglobin/GitHub-Source-Indexer
...
function Get-ActualFileName($file) {
    ...
    {        
        $parts = $file.Split('\');
        $regExpParts = $parts | Select -Skip 1 | % { "[" + [System.Char]::ToUpper($_[0]) + "]" + $_.SubString(1) }
        
        $drive = (gi $file).PSDrive.Root
        $regExp = $drive + [System.String]::Join('\', $regExpParts)
